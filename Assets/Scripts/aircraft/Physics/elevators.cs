...
public class elevators : MonoBehaviour
{
    ...
    void Update()
    {
        if(Input.GetKey("w")){angle -= turnSpeed*Time.deltaTime;}
        if(Input.GetKey("s")){angle += turnSpeed*Time.deltaTime;}
        if(Input.GetMouseButton(0) || Input.GetMouseButton(1)) {angle -= Input.GetAxis("Mouse Y")/10f;}

        angle -= angle*turnSpeed*Time.deltaTime;
        angle = Mathf.Clamp(angle,-1f,1f);

        transform.localRotation = Quaternion.Euler(new Vector3(angle*maxAngle*speedCurve.Evaluate(transform.parent.InverseTransformDirection(rb.velocity).z),0,0));
    }
    void FixedUpdate()
    {
