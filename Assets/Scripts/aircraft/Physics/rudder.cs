...
public class rudder : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        origPos = rudderObj.localPosition;
    }
    void Update()
    {
        if(Input.GetKey("a")){angle += turnSpeed*Time.deltaTime;}
        if(Input.GetKey("d")){angle -= turnSpeed*Time.deltaTime;}
        if (Input.GetMouseButton(0)) { angle -= Input.GetAxis("Mouse X") / 15f; }

        angle -= angle*turnSpeed*Time.deltaTime;
        float sc = speedCurve.Evaluate(transform.parent.InverseTransformDirection(rb.velocity).z);

        Vector3 direction = rudderObj.TransformDirection(new Vector3(0f,axis.y,axis.x));
        rudderObj.localRotation = new Quaternion();
        rudderObj.RotateAround(rudderObj.position,direction,angle*maxAngle*sc);
        rudderObj.localPosition = origPos;

        transform.localRotation = Quaternion.Euler(new Vector3(0f,angle*maxAngle*sc,0f));

        if(GetComponentInParent<flightScript>().gearDown){frontWheel.localRotation = Quaternion.Euler(new Vector3(-90f,-angle*maxAngle,0));frontWheel.GetComponent<WheelCollider>().steerAngle = -angle*maxAngle;}
    }
    void FixedUpdate()
    {
